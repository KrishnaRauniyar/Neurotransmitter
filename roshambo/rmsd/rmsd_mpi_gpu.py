...
def process_one(pdb_id, chain_id, drug_name, drug_id, pdb_dir, exclude_h):
    ...
    #  # ADD DEBUGGING HERE 
    # print(f"[DEBUG] Checking PDB {pdb_id}, chain {chain_id}, target: {drug_name} {drug_id}")
    ...
    #               "Atoms:", len(res))
    # else:
    #     print(f"[DEBUG] Chain {chain_id} not found in PDB {pdb_id}")
    # # END DEBUGGING 
    ...
    for ch in model:
        if ch.id == chain_id:
            for res in ch:
                ...
                if resnum == drug_id and resname.startswith(target):
...
def main():
    ...
    if rank == 0:
        ...
        # print("\n================ DEBUG: Parsed CSV Entries ================\n")
        # for prot, ch, name, rid in entries:
        #     print(f"CSV: {prot}_{ch}_{rid}_{name}")
        #     print(f"  pdb_id   = {prot}")
        #     print(f"  chain    = {ch}")
        #     print(f"  resnum   = {rid}")
        #     print(f"  resname  = {name}")
        #     print()
        # print("===========================================================\n")
